...
export function PongCanvas({ mode, difficulty, table, onBack, onRematch, onComplete }: PongCanvasProps) {
  ...
  return <main className="match-shell" aria-label="Partida 3D de Pring Pongas">
    ...
    <section className="canvas-wrap"><canvas ref={canvasRef} className="game-canvas game-canvas-3d" aria-label="Mesa tridimensional de pingue-pongue. Use W/S ou setas para mover a raquete." /><div className="canvas-message">{snapshot.message}</div>{paused && <div className="game-overlay"><div className="overlay-card"><span className="eyebrow">INTERVALO ESTRATÉGICO</span><h2>PAUSA</h2><p>A arena 3D está esperando o próximo caos.</p><button className="primary-button" onClick={() => window.dispatchEvent(new KeyboardEvent("keydown", { key: " " }))}><Play size={17} /> Retomar</button></div></div>}{finished && <div className="game-overlay"><div className="overlay-card result-card"><div className="trophy-icon"><Trophy size={24} /></div><span className="eyebrow">PARTIDA ENCERRADA</span><h2>{snapshot.status === "won" ? "VOCÊ AMASSOU" : "QUASE LÁ"}</h2><p>{snapshot.message}. Placar final <b>{snapshot.playerScore} x {snapshot.botScore}</b>.</p><div className="result-actions"><button className="primary-button" onClick={onRematch}><RotateCcw size={17} /> Outra partida</button><button className="ghost-button" onClick={onBack}>Voltar ao menu</button></div></div></div>}</section>
    <footer className="match-footer"><span><kbd>W</kbd><kbd>S</kbd> ou <kbd>↑</kbd><kbd>↓</kbd> mover</span><span className="match-callout"><span className="pulse-bar" /> RAQUETES 3D ATIVAS</span><button className="pause-button" onClick={() => window.dispatchEvent(new KeyboardEvent("keydown", { key: " " }))}>{paused ? <Play size={15} /> : <Pause size={15} />} {paused ? "Retomar" : "Pausar"}</button></footer>
  </main>;
}
